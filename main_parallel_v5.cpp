...
#include <random>
#include <omp.h>
#include "kmeans.h" // Struttura Point + distanceSquared()
...
int main() {
    ...
    std::cout << "K-Means Parallelo (v5 - Ottimizzato) Iniziato..." << std::endl;
    ...
    // --- 2. INIZIALIZZAZIONE CENTROIDI ---
    std::vector<Point> centroids(K);
    for (int i = 0; i < K; ++i)
        centroids[i] = points[i];
    std::cout << "Centroidi inizializzati." << std::endl;

    // --- 3. ALLOCAZIONE MEMORIA PARALLELA ---
    ...
    #pragma omp parallel
    {
        ...
        std::vector<Point> local_centroids(K);

        for (int iter = 0; iter < MAX_ITERATIONS; ++iter) {

            // --- Copia locale dei centroidi per thread ---
            #pragma omp single
            {
                // Nessuna barriera necessaria: i thread leggeranno subito la copia
            }
            ...
            for (int i = 0; i < NUM_POINTS; ++i) {
                ...
                for (int k = 0; k < K; ++k) {
                    double dist = distanceSquared(points[i], local_centroids[k]);
                    if (dist < minDistance) {
                        minDistance = dist;
                        bestCluster = k;
                    }
            ...
            // --- RIDUZIONE FINALE (solo un thread) ---
            #pragma omp single
            {
